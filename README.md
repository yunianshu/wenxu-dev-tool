...
- **项目独立配置**：项目只保存服务器选择，安装目录、服务端口、健康检查、发布历史继续各自独立。自动模式按稳定项目/目标标识分配目录与端口；高级模式可单独设置安装目录。更改服务器登录信息不会改写项目目录，也不会被旧项目表单覆盖。
...
- 每个项目、正式目标使用独立目录、Compose 项目名、服务端口与持久化数据卷。版本目录和构建镜像按发布记录区分，同版本代码更新也不会覆盖回滚镜像。全部容器与业务 HTTP 健康检查通过才记为成功。
...
- **数据同步（按目标）**：发布成功后可把本地数据目录（如 `data/`）打包上传，解压覆盖到服务器目标目录（默认 `shared/data`，跨版本共享，发布/回滚不影响数据）；上传经 SHA256 校验，同步失败时整单标记失败并提示「代码已发布但数据未同步」；生产与测试目标可分别设置，不需要的项目不启用即可
...
**数据同步结论的口径**：方案区分两种常被混为一谈的情况——`share`（数据由服务器自己产生，只需迁移到 `<安装根>/shared` 跨版本共享，**不需要**把本地数据推上去）与 `upload`（本地有服务器不会自己产生的数据，需要发布后推送）。只有后者才建议启用「数据同步」。
