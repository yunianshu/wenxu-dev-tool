...
默认使用自动正式发布：**选中项目 → 设置正式服务器地址与 SSH 登录凭据 → 点击发布**。配置只需保存一次，后续发布直接使用。无需填写远端目录、打包命令或预装本地 Java/Python/Node 构建环境。
...
- 新项目不再继承上个项目的版本、构建脚本或远端目录。「从其他项目复制」只复制服务器连接与加密凭据。已有脚本/Compose 部署继续保留，在「部署设置」中使用；旧项目可通过「设置正式服务器」切换自动发布。
...
node scripts/deploy-auto-e2e.cjs                # 正式服务器简表保存后可一键发布（不连接生产）
